export const waterVertexShader = `
  ...
  void main() {
    ...
    // Compute normal
    float eps = 0.1;
    float hx = sin((modelPosition.x + eps) * uBigWavesFrequency.x + uTime * uBigWavesSpeed) *
               sin(modelPosition.z * uBigWavesFrequency.y + uTime * uBigWavesSpeed) *
               uBigWavesElevation;
    float hz = sin(modelPosition.x * uBigWavesFrequency.x + uTime * uBigWavesSpeed) *
               sin((modelPosition.z + eps) * uBigWavesFrequency.y + uTime * uBigWavesSpeed) *
               uBigWavesElevation;
    
    vNormal = normalize(vec3(elevation - hx, eps, elevation - hz));
...
export const glowFragmentShader = `
  ...
  void main() {
    ...
    intensity *= uIntensity * (0.7 + pulse * 0.3);
...
export const fireVertexShader = `
  ...
  void main() {
    vRandom = aRandom;
    float noise = sin(aRandom * 100.0 + uTime * 3.0) * 0.1;
    vec3 pos = position;
    pos.x += noise * (1.0 - uv.y);
    pos.z += noise * (1.0 - uv.y);
    pos.y += uv.y * 0.5;
...
export const fireFragmentShader = `
  ...
  void main() {
    ...
    fireColor = mix(fireColor, vec3(1.0, 1.0, 0.9), vProgress * vProgress);
...
export const cannonBallVertexShader = `
  void main() {
    gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
  }
`;

export const cannonBallFragmentShader = `
  uniform float uTime;
  
  void main() {
    float glow = abs(sin(uTime * 10.0)) * 0.3 + 0.7;
    gl_FragColor = vec4(1.0, 0.6, 0.1, glow);
  }
`;
